...
And compare them with those existing samples' centroids, get k nearest centroids, 
and determine whether the new sample is covid or non_covid based on k nearest sample's centroid.
...
import pandas as pd
import scanpy as sc
from helper_func import n_random_sampling, count_nearest_samples, performance_eval, calc_dist_centroids, calc_k_nearest_centroids
import argparse
import warnings
...
def main(num_kfold=5, num_nearest_samples=3):
    # Read adata
    print("Starting the experiment for sample classification in scenario 2.4 with DP: PCA(X)+gaussian noise...")
    result_rep = dict()
    ...
    #for rep in ['X_umap']:
        ...
        for k in range(num_kfold):
            ...
            y_true_nearest, y_pred_nearest = calc_k_nearest_centroids(num_nearest_samples, exist_sample_centroids, new_sample_centroids, 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_nearest_samples', type=int, help='Number of k nearest neighbors', default=3)
    # Parse the arguments
    args = parser.parse_args()

    print(f"Running the experiment of scenario 2.4 DP with knn={args.num_nearest_samples}...")
    main(num_kfold=args.num_kfold, num_nearest_samples=args.num_nearest_samples)
    print(f"Done experiment of scenario 2.4 DP with knn={args.num_nearest_samples}...")
