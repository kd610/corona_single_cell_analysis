    ...
    for i in range(num_dataset_sampling):       
        ...
        for rep in ['X_pca', 'X_umap']:
            ...
            for k in tqdm(range(num_kfold)):
                ...
                #del adata
                print(adata_train.X)
                print("Run DP-PCA on the adata_train.")
                # Run PCA and neighbour graph on the adata_train. 
                sc.pp.neighbors(adata_train, n_pcs = 30, n_neighbors = 20) 
                dp_pca = PCA(n_components=30, epsilon=1.0)
                adata_train.obsm['X_pca'] = dp_pca.fit_transform(adata_train.X)
                ...
                adata_concat.write(f"./data/k{k}_{rep_name}_adata_DP_PCA_GSE_176269_COVID19.h5ad") # type: ignore
